...
Deep learning is becoming an increasingly popular approach to solve a lot of problems these days. One of the reasons for the popularity of deep learning is that it eliminates the need to perform domain expertise driven feature design. For example, in computer vision raw pixel data is used as the input to deep learning models without any pre-processing. It was shown that deep learning models are able to capture complex high level fefatures that would be far too difficult to design by a human being. 
...
The fact that there is lack of understanding of the underlying features poses the concern of how well can we trust the output of the system. Recent research \cite{szegedy_intriguing_2013} \cite{goodfellow_explaining_2014} has shown that it is possible to fool the classifier into misclassifying the input by applying an imperceptible non-random values to it. This exposed the vulnerability of deep learning and quickly became a popular area of research in security and privacy of image recognition algorithms. This concern for security exists in the voice recognition domain as well and in recent years understanding the vulnerabilities of voice recognition has also become a popular topic.

The more general conclusions that can be drawn from \cite{szegedy_intriguing_2013} and \cite{goodfellow_explaining_2014} is that deep learning models are not learning high level features as well as we expect them too. 
...
We start with the GTZAN dataset because there is previous work to compare our implementations and results to directly. 
\subsection{Project 2: Testing on well defined MIR tasks}
While working on implementing existing attacks and defenses in GTZAN we will identify a more well defined task to test the results on. Candidate tasks include instrument recognition, vocal activity detection.
