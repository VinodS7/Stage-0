...
\IEEEPARstart{M}{usic} Talk about large collections of data and where they are used. Then talk about issues and challenges that lead to anomaly detection as a solution. Then talk about challenges that lead to adversarial attacks and defenses as a solution.
...
\item For mechanical failures there are deep learning aproaches for anomaly detection, mostly inspired by the fact that it is harder to define the anomaly. 

\item Work done on anomaly detection in medicine to identify diseases from measurements automatically

\item Work done on anomaly detection on GTZAN dataset using classicalmachine learning and on mammal sound recognition
\end{enumerate}


\subsection{Adversarial attacks}
\begin{enumerate}
\item Discovery of adversarial examples and the conclusion that the cause was non-linearity
\item Transferability of adversarial examples between models and datasets
\item Different defenses against the atttacks
\item Adversarial attacks in speech and music
\end{enumerate}
...
\begin{enumerate}
\item Transferring adversarial examples in audio between models with identical inputs
\item Can adversarial examples be played over the air
\item Can you create a universal perturbation that has a high chance of making an audio an adversarial example
\item Robustness of adversarial examples to different types of compression
\end{enumerate}
